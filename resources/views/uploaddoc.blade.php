<div class="relative" wire:ignore.self>
    ...
    <div class="p-6 md:p-8 bg-gray-50 dark:bg-gray-800 shadow-md rounded-lg space-y-6">
        <form wire:submit="save" enctype="multipart/form-data" class="max-w-lg mx-auto space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-4">
                    <div class="max-w-lg mx-auto" style="width: 100% ; text-align:center">
                        ...
                        <div id="dropzone"
                            ...
                            <input type="file" id="upload" class="hidden" wire:model="files" multiple accept=".txt,.pdf,.doc,.docx,.xls,.xlsx,.csv,.ppt,.pptx,.png,.jpeg,.jpg,.gif,.mp4,.avi,.mkv,.mov,.wmv,.flv,.webm,.3gp,.m4v">
                        ...
                        <div class="w-full mt-2" id="bar-upload-file-progress" style="display: none">
                            <!-- Barre de progression principale -->
                            <div class="w-full bg-gray-200 rounded-full h-3 mb-2">
                                <div id="progress" class="bg-gradient-to-r from-blue-500 to-blue-600 h-3 rounded-full transition-all duration-300 ease-out relative"
                                    style="width: 0%;">
                            ...
                            <div class="flex justify-between items-center text-sm">
                                <div class="flex items-center space-x-2">
                                    <span id="progressText" class="font-medium text-gray-700 dark:text-gray-300">0%</span>
                                    <span id="uploadSpeed" class="text-gray-500 dark:text-gray-400"></span>
                                </div>
                                <div class="flex items-center space-x-2">
                                    <span id="fileSize" class="text-gray-500 dark:text-gray-400"></span>
                                    <span id="timeRemaining" class="text-gray-500 dark:text-gray-400"></span>
                                </div>
                            </div>
                            
                            <!-- Indicateur spécial pour vidéos -->
                            <div id="videoIndicator" class="mt-2 p-2 bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-lg text-blue-800 dark:text-blue-200 text-sm" style="display: none">
                                <div class="flex items-center">
                                    <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M8 5v14l11-7z"/>
                                    </svg>
                                    <span>Upload vidéo en cours... Cela peut prendre plus de temps selon la taille du fichier.</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end w-full mt-2">
                            @if (count($files) > 0)
                                <span id="totalFile" class="text-sm text-gray-600 dark:text-gray-400 font-medium">
                                    {{ count($files) }} fichier(s) sélectionné(s)
                                </span>
                            @endif
                            <span wire:loading wire:target="files" class="mt-1">
                        ...
                        @if ($files)
                        <div class="overflow-y-auto h-30 border rounded-lg bg-gray-50 dark:bg-gray-700 mt-4 p-2">
                            @foreach ($files as $index => $file)
                            <div class="flex justify-between items-center bg-white dark:bg-gray-600 p-2 mt-1 rounded">
                                <div class="flex items-center space-x-2">
                                    <!-- Icône selon le type de fichier -->
                                    @php
                                        $extension = strtolower($file->getClientOriginalExtension());
                                        $isVideo = in_array($extension, ['mp4', 'avi', 'mov', 'wmv', 'flv', 'webm', 'mkv', '3gp', 'm4v']);
                                        $isImage = in_array($extension, ['png', 'jpeg', 'jpg', 'gif']);
                                    @endphp
                                    ...
                                    <small class="text-blue-700 dark:text-blue-300 text-xs">
                                        {{$index+1}}. {{ $file->getClientOriginalName() }}
                                    ...
                                    <span wire:loading wire:target="removeFile({{ $index }})">
                                ...
                                <button type="button" wire:click="removeFile({{ $index }})"
                                    class="text-red-500 hover:text-red-700 font-bold text-lg leading-none">&times;
                                </button>
                            </div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    Fichiers valides : Image(PNG, JPEG), Text, PDF, Word, Excel, CSV, PowerPoint, Vidéos(MP4, AVI, MKV, MOV, WMV, FLV, WEBM, 3GP, M4V) - Limite: 1GB
                </p>
            </div>
            ...
            <div>
                <div class="flex flex-iterms">
                    <div>
                        <label class="switch">
                            <input type="checkbox" wire:model="lock" wire:change="checkLock">
                            <div class="slider">
                                <div class="circle">
                                    <svg class="cross" xml:space="preserve"
                                        style="enable-background:new 0 0 512 512" viewBox="0 0 365.696 365.696" y="0"
                    ...
                    <div>
                        <p class="text-blue-600 font-bold"> Verrouillage fichier(s)</p>
                    </div>
                </div>
                ...
                @if ($lock)
                    <div class="block max-w-full p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                        <label for="phone-input"
                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Entrer code de
                            verrouillage:</label>
                        <div class="relative">
                            ...
                            <input type="password" wire:model="code_verrouille" pattern="\d*" inputmode="numeric"
                ...
                <div>
                    ...
                    <div class="flex justify-end gap-4">
                        ...
                        <button id="annul_button" data-modal-hide="uploadFile" type="button"
    ...
    <script>
        ...
        function enregistrer() {
            ...
            if (!inputs.files || inputs.files.length < 1) {
        ...
        const bar_upload_file_progress = document.getElementById('bar-upload-file-progress');
        ...
        document.addEventListener('livewire-upload-start', () => {
            ...
            // Vérifier si il y a des vidéos
            const fileInput = document.getElementById('upload');
            if (fileInput && fileInput.files) {
                const hasVideo = Array.from(fileInput.files).some(file => isVideoFile(file.name));
                if (videoIndicator) {
                    videoIndicator.style.display = hasVideo ? 'block' : 'none';
                }
            }
        ...
        document.addEventListener('livewire-upload-progress', event => {
            ...
            if (progressText) {
                progressText.textContent = (progress === 100) ? 'Traitement...' : progress + '%';
            }
        });
        ...
        document.addEventListener('livewire-upload-finish', () => {
            ...
            setTimeout(() => {
                bar_upload_file_progress.style.display = "none";
            }, 3000);
        });
        ...
        document.addEventListener('livewire-upload-error', () => {
            ...
            if (progressElem) {
                ...
                progressElem.className = 'bg-red-500 h-3 rounded-full transition-all duration-300 ease-out relative';
            }
            if (progressText) progressText.textContent = 'Erreur d\'upload';
            if (videoIndicator) videoIndicator.style.display = 'none';
        });
        ...
        dropzone.addEventListener('dragover', () => dropzone.classList.add('grossirDrag'));
        dropzone.addEventListener('dragleave', () => dropzone.classList.remove('grossirDrag'));
        dropzone.addEventListener('drop', () => dropzone.classList.remove('grossirDrag'));
        ...
        dropzone.addEventListener('drop', function(e) {
            ...
            if (files.length) {
                ...
                [...inputs.files].forEach(file => dataTransfer.items.add(file));
